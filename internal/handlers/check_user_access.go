...
func (h *Handler) CheckUserAccess(ctx *gin.Context) {

	var request models.CheckUserAccessRequest
	...
	response := make(chan models.CheckUserAccessResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.ICheckAccessService.CheckAccessUser(request, response)

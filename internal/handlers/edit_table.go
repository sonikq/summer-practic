...
func (h *Handler) EditTable(ctx *gin.Context) {

	var request models.EditTableRequest
	...
	response := make(chan models.EditTableResponse, 1)

	c, cancel := context.WithTimeout(ctx, time.Second*time.Duration(h.config.CtxTimeout))
	defer cancel()

	go h.service.IEditTableService.EditTable(request, response)
